...
<html>
  <head>
    ...
    <title>My JSP 'mybookingdetail.jsp' starting page</title>
    		...
    		<fieldset>
    			...
    			<table>
    				...
    				<tr>
    					...
    					<c:choose>
    						<c:when test="${requestScope.meeting.canceledtime==null }">
    						<td><input type="submit" value="取消会议"></td>
    						</c:when>
    						<c:otherwise>
    						<c:if test="${currentTime>requestScope.meeting.canceledtime }">
    						<td><input type="button" value="已取消"></td>
    						</c:if>
    						<c:if test="${!(currentTime>requestScope.meeting.canceledtime )}">
    						<td><input type="button" value="取消会议"></td>
    						</c:if>
    						</c:otherwise>
    					</c:choose>
